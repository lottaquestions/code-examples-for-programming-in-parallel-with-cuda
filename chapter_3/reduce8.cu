...
// To compile: nvcc -I/home/lottaquestions/nvidia-installers/cuda-samples/Common -G -o reduce8.bin reduce8.cu

namespace cg = cooperative_groups;

__global__ void reduce8(r_Ptr<float> sums, cr_Ptr<float> data, int n){
    // This kernel assumes arrays sums is set to zero on entry and that
    // blockSize is a multiple of 32
    auto grid = cg::this_grid();
    auto block = cg::this_thread_block();
    auto warp = cg::tiled_partition<32>(block);

    // Accumululate threads sums in register variable
    float v = 0.0f;
