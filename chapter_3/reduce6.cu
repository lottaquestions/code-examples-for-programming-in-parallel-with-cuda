...
// To compile: nvcc -I/home/lottaquestions/nvidia-installers/cuda-samples/Common -G -o reduce6.bin reduce6.cu

namespace cg = cooperative_groups;

template<int blockSize> __global__ void reduce6(r_Ptr<float> sums, cr_Ptr<float> data, int n){
    // This template kernel assumes blockDim.x = blockSize, and that blockSize <= 1024
    __shared__ float s[blockSize];

    // cg definitions for launch config
    ...
    // Simplified thread linear addressing loop
    ...
    {
        s[id] += s[id + 32]; // Cannot use shfl_down yet since this line reads from a different warp
        warp.sync();
